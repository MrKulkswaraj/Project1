...
public class AuthenticationController {
	...
	@PostMapping("/Auth")
	public String authenticateUser(@RequestParam("username") String name, @RequestParam("password") String password) {
		User user = authService.GetUserByName(name);
		logger.info(user.getName() + " attempted to login with " + user.getPassword());
		String path = (authService.isValidPassword(password, user.getPassword())) ? "success" : "failure";
